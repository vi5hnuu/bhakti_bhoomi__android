...
class _RamcharitmanasVersesScreenState extends State<RamcharitmanasVersesScreen> {
  ...
  initState() {
    super.initState();
    token = loadVerse(kand: widget.kand, verseNo: 1);
  }

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<RamcharitmanasBloc, RamcharitmanasState>(
      buildWhen: (previous, current) => previous.allVerses != current.allVerses,
      builder: (context, state) => Scaffold(
          ...
          body: PageView.builder(
            ...
            itemBuilder: (context, index) {
              final verse = state.getVerse(kand: widget.kand, verseNo: index + 1);
              return Center(
                child: Padding(
            ...
            onPageChanged: (pageNo) => setState(
              () {
                ...
                token = loadVerse(kand: widget.kand, verseNo: pageNo + 1);
              },
            ),
          )),
    );
  ...
  CancelToken loadVerse({required String kand, required int verseNo}) {
    CancelToken cancelToken = CancelToken();
    BlocProvider.of<RamcharitmanasBloc>(context).add(FetchRamcharitmanasVerseByKandaAndVerseNo(kanda: kand, verseNo: verseNo, cancelToken: cancelToken));
    return cancelToken;
  }
